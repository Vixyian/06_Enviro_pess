...
$tyopeOfIncident = $_POST["tyopeOfIncident"];
...
if($btnDispatchClicked == true){
	...
	$patrolcarDispatch = [];
	$numOfpatrolCarDispatch = 0;
	if($hasCarSelection == true){
		$patrolcarDispatch = $_POST["cbCarSelection"];
		$numOfpatrolCarDispatch = count($patrolcarDispatch);
	}
	$incidentStatus = 0;
	if ($numOfpatrolCarDispatch > 0){
		$incidentStatus = 2; // dispatch
	} else {
	...
	$sql= "INSERT INTO `incident`(`caller_name`, `phone_number`, `incident_type_id`, `incident_location`, `incident_desc`, `incident_status_id`, `time_called`) VALUES ('".$callerName."','".$contactNo."','".$tyopeOfIncident."','".$locationOfIncident."','".$descriptionOfIncident."','".$incidentStatus."',now())";
	//echo $sql;	
	$conn = new mysqli(DB_SERVER,DB_USER,DB_PASSWORD,DB_DATABASE);
	...
	foreach($patrolcarDispatch as $eachCarId){
		// echo $eachCarId . "<br>";
		...
		$sql = "INSERT INTO `dispatch`(`incident_id`, `patrolcar_id`, `time_dispatch`) VALUES (" . $incidentId . ",'" . $eachCarId . "',now())";
...
<section class="mt-3">
  <form action="<?php echo htmlentities ($_SERVER["PHP_SELF"]) ?>" method ="post">
    ...
    <div class="col-sm-8">
	  ...
	  <div class="form-group row">
    <label for="tyopeOfIncident"  class="col-sm-4 col-form-label">Type of Incident</label>
    <div class="col-sm-8">
		<span>
		<?php echo $tyopeOfIncident; ?>
		<input id="tyopeOfIncident" type="hidden" name="tyopeOfIncident" value="<?php echo $tyopeOfIncident; ?>">
		</span>
    </div>
    </div>
